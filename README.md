...
这是一个面向中医方剂、药材、症状、功效与图谱证据的全栈问答系统。当前版本在原有 Neo4j + FAISS 图谱问答链路之上，加入了 `Redis + PostgreSQL` 共享问答缓存、首轮语义改写短路、明显中医意图快速识别、LLM 并发限流与超时保护等优化，用于减少不必要的大模型调用、沉淀高频问答、提升多会话并发稳定性，并在缓存或加速路径不可用时自动回退到知识图谱主链路。
...
- 管理员知识入库支持方药资料识别、3D 图谱预览、`1跳/2跳` 深度切换、正式导入与删除，并尽量避免普通操作触发全量向量重建。
...
- 首轮、无历史上下文的问题可直接复用原始输入，跳过语义改写 LLM 调用。
- 明显属于中医领域的问题优先走本地规则判断意图，只有不确定时才调用意图识别模型。
...
- 首轮语义改写短路、意图快速判断、LLM 并发限流和超时保护共同减少慢请求堆积。
- 管理员后台知识入库预览保留原 AI 图谱作为兜底，`1跳/2跳` 按钮优先复用图谱页同一接口。

## 问答链路加速与稳定性

当前问答链路不是所有步骤都无条件调用大模型，而是按“能本地判断就本地判断、能缓存命中就缓存命中、不能确定再进入主链路”的原则执行。

### 共享缓存优先

- 日常问候和高频问答先查 Redis 精确热缓存。
- Redis 未命中时继续查 PostgreSQL 精确缓存。
- 精确缓存未命中后再做 PostgreSQL 相似问答检索。
- 中等相似度问题才调用轻量模型做 JSON 二次确认。
- 仍未命中时，才进入 Neo4j + LangGraph 主问答链路。

### 语义改写短路

语义改写节点位于 `__004__langgraph_more_nodes/nodes/semantic_transcription_node.py`。

- 如果当前问题是首轮问题，且没有历史上下文需要补全，系统直接使用用户原始输入作为语义转写结果。
- 这样可以省掉一次语义改写 LLM 调用，减少首轮问答等待时间。
- 一旦存在历史上下文，仍会进入语义改写节点，让模型结合上下文澄清用户真实意图。

### 中医意图快速判断

中医意图节点位于 `__004__langgraph_more_nodes/nodes/zhongyi_intent_node.py`。

- 命中“中医、方剂、药材、功效、主治、归经、针灸、本草、伤寒论”等明显关键词时，直接判定为中医相关问题。
- 命中“什么药、吃什么、怎么用药”等典型问法时，直接进入中医问答链路。
- 短文本以“汤、丸、散、饮、膏、丹、剂、露”等常见方剂后缀结尾时，优先按中医意图处理。
- 只有规则不能确定时，才调用意图识别 LLM，避免每个请求都经过一次分类模型。

### LLM 并发、超时与降级

公共 LLM 适配层位于 `common/llm.py`。

- `LLM_API_CONCURRENCY` 控制同时进入供应商接口的大模型请求数量，默认值为 `3`。
- `LLM_API_TIMEOUT_SECONDS` 控制非流式调用总超时，默认值为 `45` 秒。
- `LLM_API_STREAM_IDLE_TIMEOUT_SECONDS` 控制流式输出的分块空闲超时，默认跟随 `LLM_API_TIMEOUT_SECONDS`。
- 同步 LangGraph 节点逐步迁移到 `llm_ainvoke`、`llm_astream`、`llm_runnable_astream` 等统一异步封装，避免单个模型请求长期挂起拖住任务状态。

### 图谱与模型预热

- FastAPI 启动时会预热实体抽取模型、embedding 模型、FAISS index 与 `id2text` 映射。
- 预热失败不会阻塞服务启动，但相关能力会在运行时体现错误，便于排查部署资产是否缺失。
- 知识入库新增实体时走增量索引追加；删除实体时屏蔽索引映射，避免日常后台操作触发全量向量重建。
...
LLM_API_STREAM_IDLE_TIMEOUT_SECONDS=45
...
管理员后台提供方药知识的识别、预览、确认导入和删除能力。

- `AI识别` 只生成结构化草稿和右侧 3D 图谱预览，不直接写入 Neo4j。
- `确认导入图谱` 才会正式写入 Neo4j，并生成后台操作记录。
- 右侧预览区提供 `1跳/2跳` 按钮。系统会优先请求与图谱页一致的 `/api/graph?depth=1/2` 数据；如果当前方药尚未正式入库或图谱接口没有返回数据，则回退到 AI 识别阶段生成的预览图谱。
- 删除方药会从 Neo4j 删除对应实体，并同步屏蔽实体索引映射，避免检索继续命中已删除实体。
- 后台用户管理和操作记录中的时间按上海时间展示。
...
```powershell
D:\Anaconda\envs\KG\python.exe -m pytest tests/test_async_llm_chain.py -q
```
